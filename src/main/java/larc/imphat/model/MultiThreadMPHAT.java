...
public class MultiThreadMPHAT {
		...
		private void updateOptimalParams(int startIndex, int endIndex) {
			for (int u = startIndex; u < endIndex; u++) {
				...
				for (int p = 0; p < Configure.NUM_OF_PLATFORM; p++) {
					currUser.optPlatformPreference[p] = currUser.platformPreference[p];
				}
			}
		}
			...
			for (int u = startIndex; u < endIndex; u++) {
				...
				for (int p = 0; p < Configure.NUM_OF_PLATFORM; p++) {
					...
					if (currUser.platforms[p] == 1) {
						...
						currUser.platformPreference[p] = g.sample();
					} else {
						currUser.platformPreference[p] = Double.NEGATIVE_INFINITY;
					}
					//
					currUser.relativePlatformPreference = MathTool.softmax(currUser.platformPreference);
				}
			}
		}
		...
		private void gibbsInitAuthorityHubPlatformPreference(int startIndex, int endIndex) {
			...
			for (int u = startIndex; u < endIndex; u++) {
				...
				if (InitPlatformPreferenceByTopicModeling) {
					...
					double min = Double.POSITIVE_INFINITY;
					for (int p = 0; p < Configure.NUM_OF_PLATFORM; p++) {
						if (min > currUser.platformPreference[p]) {
							min = currUser.platformPreference[p];
						}
					}
					norm = 1 / min + 0.1;
					...
					for (int p = 0; p < Configure.NUM_OF_PLATFORM; p++) {
						if (currUser.platforms[p] == 1) {
							currUser.platformPreference[p] = Math.log(currUser.platformPreference[p] * norm);
						} else {
							currUser.platformPreference[p] = Double.NEGATIVE_INFINITY;
						}
					}
		...
		for (int u = 0; u < dataset.nUsers; u++) {
			...
			if (currUser.followings != null) {
				for (int i = 0; i < currUser.followings.length; i++) {
					...
					double HupAvp = 0;
					for (int z = 0; z < nTopics; z++) {
						HupAvp += currUser.hubs[z] * currUser.relativePlatformPreference[p] * following.authorities[z]
								* following.relativePlatformPreference[p];
					}
					HupAvp = HupAvp * lamda;
			...
			if (currUser.followers != null) {
				for (int i = 0; i < currUser.followers.length; i++) {
					...
					double HupAvp = 0;
					for (int z = 0; z < nTopics; z++) {
						HupAvp += follower.hubs[z] * follower.relativePlatformPreference[p] * currUser.authorities[z]
								* currUser.relativePlatformPreference[p];
			...
			if (currUser.nonFollowings != null) {
				for (int i = 0; i < currUser.nonFollowings.length; i++) {
					...
					double HupAvp = 0;
					for (int z = 0; z < nTopics; z++) {
						HupAvp += currUser.hubs[z] * currUser.relativePlatformPreference[p]
								* nonFollowing.authorities[z] * nonFollowing.relativePlatformPreference[p];
			...
			if (currUser.nonFollowers != null) {
				for (int i = 0; i < currUser.nonFollowers.length; i++) {
					...
					double HupAvp = 0;
					for (int z = 0; z < nTopics; z++) {
						HupAvp += nonFollower.hubs[z] * nonFollower.relativePlatformPreference[p]
								* currUser.authorities[z] * currUser.relativePlatformPreference[p];
			...
			double platformDenominators = 0;
			for (int p = 0; p < Configure.NUM_OF_PLATFORM; p++) {
				if (currUser.platforms[p] == 0) {
					continue;
				}
				platformDenominators += Math.exp(currUser.platformPreference[p]);
			}
			platformDenominators = Math.log(platformDenominators);
			...
			for (int s = 0; s < currUser.nPosts; s++) {
				if (currUser.postBatches[s] == batch) {
					...
					// platform
					postPlatformLikelihood += currUser.platformPreference[currPost.platform] - platformDenominators;

					if (Double.isInfinite(postPlatformLikelihood) || Double.isNaN(postPlatformLikelihood)) {
						System.out.printf("[Post] platform[%d] = %.12f\n", currPost.platform,
								currUser.platformPreference[currPost.platform]);
						System.exit(0);
					}
	...
	public static double getLikelihood(int u) {
		...
		if (currUser.nonFollowings != null) {
			for (int i = 0; i < currUser.nonFollowings.length; i++) {
				...
				double HupAvp = 0;
				for (int z = 0; z < nTopics; z++) {
					HupAvp += currUser.hubs[z] * currUser.relativePlatformPreference[p] * nonFollowing.authorities[z]
							* nonFollowing.relativePlatformPreference[p];
		...
		if (currUser.nonFollowers != null) {
			for (int i = 0; i < currUser.nonFollowers.length; i++) {
				...
				double HupAvp = 0;
				for (int z = 0; z < nTopics; z++) {
					HupAvp += nonFollower.hubs[z] * nonFollower.relativePlatformPreference[p] * currUser.authorities[z]
							* currUser.relativePlatformPreference[p];
	...
	private static double getLikelihood_authority(int v, double[] x) {
		...
		if (currUser.followers != null) {
			for (int i = 0; i < currUser.followers.length; i++) {
				...
				double HupAvp = 0;
				for (int z = 0; z < nTopics; z++) {
					HupAvp += follower.hubs[z] * follower.relativePlatformPreference[p] * x[z]
							* currUser.relativePlatformPreference[p];// now
					// A_v
					// is
					// x
				}
		...
		if (currUser.nonFollowers != null) {
			for (int i = 0; i < currUser.nonFollowers.length; i++) {
				...
				double HupAvp = 0;
				for (int z = 0; z < nTopics; z++) {
					HupAvp += nonFollower.hubs[z] * nonFollower.relativePlatformPreference[p] * x[z]
							* currUser.relativePlatformPreference[p];// now
					// A_v
					// is
					// x
	...
	private static double gradLikelihood_authority(int v, int k, double x) {
		...
		if (currUser.followers != null) {
			for (int i = 0; i < currUser.followers.length; i++) {
				...
				for (int z = 0; z < nTopics; z++) {
					if (z == k) {
						HupAvp += follower.hubs[z] * follower.relativePlatformPreference[p] * x
								* currUser.relativePlatformPreference[p];
					} else {
						HupAvp += follower.hubs[z] * follower.relativePlatformPreference[p] * currUser.authorities[z]
								* currUser.relativePlatformPreference[p];
				...
				// * currUser.topicalPlatformPreference[k][p]);
				temp = Math.exp(-HupAvp);
				followerLikelihood += (1 / (1 - temp) * -temp * -lamda * follower.relativePlatformPreference[p]
						* follower.hubs[k] * currUser.relativePlatformPreference[p])
						- (1 / (temp + 1) * temp * -lamda * follower.relativePlatformPreference[p] * follower.hubs[k]
								* currUser.relativePlatformPreference[p]);
			}
		}
		...
		if (currUser.nonFollowers != null) {
			for (int i = 0; i < currUser.nonFollowers.length; i++) {
				...
				for (int z = 0; z < nTopics; z++) {
					if (z == k) {
						HupAvp += nonFollower.hubs[z] * nonFollower.relativePlatformPreference[p] * x
								* currUser.relativePlatformPreference[p];
					} else {
						HupAvp += nonFollower.hubs[z] * nonFollower.relativePlatformPreference[p]
								* currUser.authorities[z] * currUser.relativePlatformPreference[p];
				...
				temp = Math.exp(-HupAvp);
				nonFollowerLikelihood += (-lamda * currUser.relativePlatformPreference[p] * nonFollower.hubs[k]
						* nonFollower.relativePlatformPreference[p])
	...
	private static double getLikelihood_hub(int u, double[] x) {
		...
		if (currUser.followings != null) {
			for (int i = 0; i < currUser.followings.length; i++) {
				...
				double HupAvp = 0;
				for (int z = 0; z < nTopics; z++) {
					HupAvp += x[z] * currUser.relativePlatformPreference[p] * following.authorities[z]
							* following.relativePlatformPreference[p];// now
					// H_u
					// is
					// x
				}
		...
		if (currUser.nonFollowings != null) {
			for (int i = 0; i < currUser.nonFollowings.length; i++) {
				...
				double HupAvp = 0;
				for (int z = 0; z < nTopics; z++) {
					HupAvp += x[z] * currUser.relativePlatformPreference[p] * nonFollowing.authorities[z]
							* nonFollowing.relativePlatformPreference[p];// now
					// H_u
					// is
					// x
				}
	...
	private static double gradLikelihood_hub(int u, int k, double x) {
		...
		if (currUser.followings != null) {
			for (int i = 0; i < currUser.followings.length; i++) {
				...
				for (int z = 0; z < nTopics; z++) {
					if (z == k) {
						HupAvp += x * currUser.relativePlatformPreference[p] * following.authorities[z]
								* following.relativePlatformPreference[p];
					} else {
						HupAvp += currUser.hubs[z] * currUser.relativePlatformPreference[p] * following.authorities[z]
								* following.relativePlatformPreference[p];
				...
				followingLikelihood += (1 / (1 - temp) * -temp
						* (-lamda * following.authorities[k] * following.relativePlatformPreference[p]
								* currUser.relativePlatformPreference[p]))
						- (1 / (temp + 1) * temp * (-lamda * following.authorities[k]
								* following.relativePlatformPreference[p] * currUser.relativePlatformPreference[p]));
			}
		}
		...
		if (currUser.nonFollowings != null) {
			for (int i = 0; i < currUser.nonFollowings.length; i++) {
				...
				for (int z = 0; z < nTopics; z++) {
					if (z == k) {
						HupAvp += x * currUser.relativePlatformPreference[p] * nonFollowing.authorities[z]
								* nonFollowing.relativePlatformPreference[p];
					} else {
						HupAvp += currUser.hubs[z] * currUser.relativePlatformPreference[p]
								* nonFollowing.authorities[z] * nonFollowing.relativePlatformPreference[p];
				...
				temp = Math.exp(-HupAvp);
				nonFollowingLikelihood += (-lamda * currUser.relativePlatformPreference[p] * nonFollowing.authorities[k]
						* nonFollowing.relativePlatformPreference[p])
						- ((1 / (temp + 1)) * temp * (-lamda * nonFollowing.authorities[k]
								* nonFollowing.relativePlatformPreference[p] * currUser.relativePlatformPreference[p]));
	...
	/***
	 * compute likelihood of data as a function of platform preference of u when
	 * the preference is x, i.e., if L(data|parameters) = f(Eta_u) +
	 * const-of-Eta_u then this function returns f(x)
	 * 
	 * @param u
	 * @return
	 */
	private static double getLikelihood_platformPreference(int u, double[] x) {
		...
		// Set the current user to be v
		User currUser = dataset.users[u];
		double[] relativePlatformPreference = MathTool.softmax(x);
		// *********link part******
		if (useLinkInLearningPlatformPreference) {
			...
			if (currUser.followings != null) {
				for (int i = 0; i < currUser.followings.length; i++) {
					...
					double HupAvp = 0;
					for (int z = 0; z < nTopics; z++) {
						HupAvp += currUser.hubs[z] * relativePlatformPreference[p] * following.authorities[z]
								* following.relativePlatformPreference[p];
			...
			if (currUser.nonFollowings != null) {
				for (int i = 0; i < currUser.nonFollowings.length; i++) {
					...
					double HupAvp = 0;
					for (int z = 0; z < nTopics; z++) {
						HupAvp += currUser.hubs[z] * relativePlatformPreference[p] * nonFollowing.authorities[z]
								* nonFollowing.relativePlatformPreference[p];
					}
					HupAvp = HupAvp * lamda;
			...
			if (currUser.nonFollowers != null) {
				for (int i = 0; i < currUser.nonFollowers.length; i++) {
					...
					for (int z = 0; z < nTopics; z++) {
						HupAvp += nonFollower.hubs[z] * nonFollower.relativePlatformPreference[p]
								* currUser.authorities[z] * relativePlatformPreference[p];
					}
					HupAvp = HupAvp * lamda;
					nonLinkLikelihood += log2 - HupAvp - Math.log(Math.exp(-HupAvp) + 1);
				}
	 ...
	 * preference of u when the preference is x, i.e., if L(data|parameters) =
	 * f(Eta_uj) + const-of-Eta_uj then this function return df/dEta_uj at
	 * Eta_uj = x
	 * 
	 * @param u
	 * @param x
	 * @return
	 */
	private static double gradLikelihood_platformPreference(int u, int j, double x) {
		...
		for (int p = 0; p < Configure.NUM_OF_PLATFORM; p++) {
			...
			if (p == j) {
				...
				tempExps[p] = Math.exp(currUser.platformPreference[p]);
			}
			sumExp += tempExps[p];
		}
		double sumExpSqr = sumExp * sumExp;
		double[] relativePlatformPreference = new double[Configure.NUM_OF_PLATFORM];
		for (int p = 0; p < Configure.NUM_OF_PLATFORM; p++) {
			if (currUser.platforms[p] == 0) {
				relativePlatformPreference[p] = 0;
			} else {
				relativePlatformPreference[p] = tempExps[p] / sumExp;
			}
		}
		...
		if (useLinkInLearningPlatformPreference) {
				...
				for (int i = 0; i < currUser.followings.length; i++) {
					...
					for (int z = 0; z < nTopics; z++) {
						linkLikelihood += ((1 / (1 - tempExpHA)) * (-tempExpHA)
								* (-lamda * currUser.hubs[z] * following.authorities[z]
										* following.relativePlatformPreference[p] * tempGrad))
			...
			if (currUser.followers != null) {
				for (int i = 0; i < currUser.followers.length; i++) {
					...
					for (int z = 0; z < nTopics; z++) {
						linkLikelihood += ((1 / (1 - tempExpHA)) * (-tempExpHA)
								* (-lamda * follower.hubs[z] * follower.relativePlatformPreference[p]
										* currUser.authorities[z] * tempGrad))
								- (1 / (tempExpHA + 1) * tempExpHA * (-lamda * follower.hubs[z]
										* follower.relativePlatformPreference[p] * currUser.authorities[z] * tempGrad));
					}
			...
			if (currUser.nonFollowings != null) {
				for (int i = 0; i < currUser.nonFollowings.length; i++) {
					...
					tempGrad /= sumExpSqr;
					for (int z = 0; z < nTopics; z++) {
						nonLinkLikelihood += (-lamda * currUser.hubs[z] * nonFollowing.authorities[z]
								* nonFollowing.relativePlatformPreference[p] * tempGrad)
								- ((1 / (tempExpHA + 1)) * tempExpHA
										* (-lamda * currUser.hubs[z] * nonFollowing.authorities[z]
												* nonFollowing.relativePlatformPreference[p] * tempGrad));
					}
				}
			}
			if (currUser.nonFollowers != null) {
				for (int i = 0; i < currUser.nonFollowers.length; i++) {
					...
					tempGrad /= sumExpSqr;
					for (int z = 0; z < nTopics; z++) {
						nonLinkLikelihood += (-lamda * nonFollower.hubs[z] * nonFollower.relativePlatformPreference[p]
								* currUser.authorities[z] * tempGrad)
								- ((1 / (tempExpHA + 1)) * tempExpHA
										* (-lamda * nonFollower.hubs[z] * nonFollower.relativePlatformPreference[p]
												* currUser.authorities[z] * tempGrad));
					}
				}
	 ...
	 * @param u
	 */
	private static void altOptimize_PlatformPreference(int u) {
		double[] grad = new double[Configure.NUM_OF_PLATFORM];
		double[] currentX = dataset.users[u].platformPreference;
		double[] x = new double[Configure.NUM_OF_PLATFORM];

		double currentF = 0 - getLikelihood_platformPreference(u, currentX);
		...
		for (int iter = 0; iter < maxIteration_platformPreference; iter++) {
			...
			for (int p = 0; p < Configure.NUM_OF_PLATFORM; p++) {
				if (dataset.users[u].platforms[p] == 0) {
					...
					grad[p] = 0 - gradLikelihood_platformPreference(u, p, currentX[p]);
				}
			}
			...
			for (int lineSearchIter = 0; lineSearchIter < lineSearch_MaxIterations; lineSearchIter++) {
				...
				// compute f at the new x
				f = 0 - getLikelihood_platformPreference(u, x);
			...
			if (flag) {// line search successful
				...
				if (opt_platform_verbose) {
					System.out.printf("alt_platform: u = %d iter = %d f = %f\n", u, iter, f);
				}
			} else {
				// to see if F actually reduce after every iteration
				if (opt_platform_verbose) {
		...
		if (isChanged) {
			dataset.users[u].relativePlatformPreference = MathTool.softmax(dataset.users[u].platformPreference);
		}
	}
	 ...
	 * @param p
	 */
	public void gradCheck_PlatformPreference(int u, int p) {
		if (dataset.users[u].platforms[p] == 0) {
		...
		for (int i = 0; i < Configure.NUM_OF_PLATFORM; i++) {
			x[i] = dataset.users[u].platformPreference[i];
		}

		double f = getLikelihood_platformPreference(u, x);
		double g = gradLikelihood_platformPreference(u, p, x[p]);

		for (int i = 1; i <= 20; i++) {
			// reduce DELTA
			DELTA *= 0.1;
			x[p] += DELTA;
			double DELTAF = getLikelihood_platformPreference(u, x);
			double numGrad = (DELTAF - f) / DELTA;
			System.out.printf(
					String.format("[Hub] u = %d p = %d DELTA = %f numGrad = %f grad = %f\n", u, p, DELTA, numGrad, g));
			// if grad function is implemented properly, we will see numGrad
			// gets closer to grad
			x[p] -= DELTA;
	...
	private void initByGroundTruth(String groundtruthPath) {
		...
		for (int u = 0; u < dataset.users.length; u++) {
			...
			for (int p = 0; p < Configure.NUM_OF_PLATFORM; p++) {
				dataset.users[u].platformPreference[p] = dataset.users[u].groundtruth_PlatformPreference[p];
			}
			dataset.users[u].relativePlatformPreference = MathTool.softmax(dataset.users[u].platformPreference);
		}
		altOptimize_topics();
	...
	private void initPlatformPreferenceByTopicModeling() {
		...
		for (int u = 0; u < dataset.nUsers; u++) {
			User currUser = dataset.users[u];
			for (int p = 0; p < Configure.NUM_OF_PLATFORM; p++) {
				currUser.platformPreference[p] = 0;
			}
		}
		...
		for (int u = 0; u < dataset.nUsers; u++) {
			...
			for (int n = 0; n < currUser.posts.length; n++) {
				...
				currUser.platformPreference[p] += 1;
			}
		}
		...
		for (int u = 0; u < dataset.nUsers; u++) {
			...
			for (int k = 0; k < nTopics; k++) {
				...
				for (int p = 0; p < Configure.NUM_OF_PLATFORM; p++) {
					if (currUser.platforms[p] == 1) {
						if (currUser.platformPreference[p] < 1) {
							currUser.platformPreference[p] = epsilon;
						} else {
							currUser.platformPreference[p] = currUser.platformPreference[p] / currUser.nPosts;
						}
					} else {
						currUser.platformPreference[p] = Double.NEGATIVE_INFINITY;
					}
	...
	private void init() {
		...
		for (int u = 0; u < dataset.nUsers; u++) {
			...
			currUser.platformPreference = new double[Configure.NUM_OF_PLATFORM];
			...
			currUser.optPlatformPreference = new double[Configure.NUM_OF_PLATFORM];
		}
	...
	public void altCheck_PlatformPreference(int u) {
		altOptimize_PlatformPreference(u);
	}
	...
	public void output_platformPreference() {
		try {
			...
			for (int u = 0; u < dataset.nUsers; u++) {
				...
				String text = currUser.userId;
				for (int p = 0; p < Configure.NUM_OF_PLATFORM; p++) {
					text = text + "," + Double.toString(currUser.optPlatformPreference[p]);
				}
				fo.write(text + "\n");
			}
			fo.close();
	...
	public static void main(String[] args) {
		...
		int p = rand.nextInt(Configure.NUM_OF_PLATFORM);
