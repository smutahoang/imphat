...
import larc.imphat.model.Configure.ModelMode;
import larc.imphat.tool.MathTool;
import larc.imphat.tool.StatTool;
...
public class Synthetic {
	...
	private double[][] genUserPlatformPreference(int nUsers, int nTopics, int nPlatforms, int[][] userActivePlatforms) {
		double[][] userPlatformPreference = new double[nUsers][];
		for (int u = 0; u < nUsers; u++) {
			userPlatformPreference[u] = statTool.sampleDirichletSkew(alpha, nPlatforms, platformSkeness, mass, rand);

			double min = Double.POSITIVE_INFINITY;
			for (int p = 0; p < nPlatforms; p++) {
				if (min > userPlatformPreference[u][p]) {
					min = userPlatformPreference[u][p];
				}
			}
			double norm = 1 / min + 0.1;

			for (int p = 0; p < nPlatforms; p++) {
				userPlatformPreference[u][p] = Math.log(norm * userPlatformPreference[u][p]);
			}
			...
			for (int p = 0; p < nPlatforms; p++) {
				if (userActivePlatforms[u][p] == 0) {
					userPlatformPreference[u][activePlatform] += userPlatformPreference[u][p];
					userPlatformPreference[u][p] = Double.NEGATIVE_INFINITY;
				}
			}
	...
	private Post genPost(double[] userInterest, double[] platformPreference, double[][] topics) {
		...
		if (mode == ModelMode.TWITTER_LDA) {
			...
			post.topic = z;
			// platform
			post.platform = statTool.sampleMult(platformPreference, false, rand);
			// words
	...
	private double getLinkLikelihood(int nTopics, double[] userAuthority, double[] vPlatformPreference,
		...
		double prod = 0;
		for (int z = 0; z < nTopics; z++) {
			prod += userHub[z] * uPlatformPreference[platform] * userAuthority[z] * vPlatformPreference[platform];
		}
	...
	private HashMap<Integer, HashMap<Integer, HashSet<Integer>>> genNetwork(int nUsers, int nTopics, int nPlatforms,
		...
		double[][] userRelativePlatformPreference = new double[nUsers][];
		for (int u = 0; u < nUsers; u++) {
			userRelativePlatformPreference[u] = MathTool.softmax(userPlatformPreference[u]);
	...
	private void genAndsaveTweet(String outputpath, int nUsers, int nTopics, double[][] userLatentFactors,
		...
		try {
			...
			for (int u = 0; u < nUsers; u++) {
				...
				double[] uPlatformPreference = MathTool.softmax(userPlatformPreference[u]);
	...
	private void saveGroundTruth(double[][] topics, double[][] userInterest, double[][] userAuthorities,
		...
		try {
			...
			for (int u = 0; u < userPlatformPreference.length; u++) {
				bw.write(String.format("%d", u));
				for (int p = 0; p < userPlatformPreference[u].length; p++) {
					bw.write(String.format(",%f", userPlatformPreference[u][p]));
				}
				bw.write("\n");
	...
	public void genData(int nUsers, int nPlatforms, int nTopics, int nWords, String outputPath) {
		...
		double[][] userPlatformPreference = genUserPlatformPreference(nUsers, nTopics, nPlatforms, userActivePlatforms);
	...
	public static void main(String[] args) {
		...
		// generator.testTuple();
		generator.genData(1000, 2, 10, 10000, "E:/code/java/imphat/data/syn");
